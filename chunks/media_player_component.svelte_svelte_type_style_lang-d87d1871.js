import { o as noop, r as safe_not_equal, c as create_ssr_component, a as compute_rest_props, b as spread, e as escape_object, i as add_attribute, j as add_classes, v as validate_component, m as missing_component, d as escape_attribute_value, l as createEventDispatcher, t as get_store_value } from "./index-099ba245.js";
...
let selected = "youngswurve.app";
let _selected = "youngswurve.app";
let selected_media = writable(selected);
let selected_poster = writable(_selected);
let cookie = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imp1bm92ZnJ5bGlsZGJ6cXZ3b2F4Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTY0NzQwMTAyOSwiZXhwIjoxOTYyOTc3MDI5fQ.YRIlrai7CJju3v1ZXnvrmHolCprYt73FbfgreQ-u89c";
let selected_cookie = writable(cookie);
...
const ClickableTile = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.href === void 0 && $$bindings.href && href !== void 0)
    $$bindings.href(href);
  return `${validate_component(Link$1, "Link").$$render($$result, Object.assign($$restProps, { disabled }, {
...
const Scrim$1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.getWebComponent === void 0 && $$bindings.getWebComponent && getWebComponent !== void 0)
    $$bindings.getWebComponent(getWebComponent);
  return `<vm-scrim${add_attribute("class", className, 0)}${add_attribute("style", style, 0)}${add_attribute("gradient", gradient, 0)}${add_attribute("is-video-view", isVideoView, 0)}${add_attribute("is-controls-active", isControlsActive, 0)}${add_attribute("this", __ref, 0)}>${slots.default ? slots.default({}) : ``}</vm-scrim>`;
});
define$1("vm-scrubber-control", VmScrubberControl);
define$1("vm-slider", VmSlider);
define$1("vm-tooltip", VmTooltip);
...
define$1("vm-video", VmVideo);
define$1("vm-file", VmFile);
const Video$1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let __ref;
  createEventDispatcher();
  let { class: className = "" } = $$props;
  let { style = "" } = $$props;
  let { willAttach = false } = $$props;
  let { hasCustomTextManager = false } = $$props;
  ...
  if ($$props.style === void 0 && $$bindings.style && style !== void 0)
    $$bindings.style(style);
  if ($$props.willAttach === void 0 && $$bindings.willAttach && willAttach !== void 0)
    $$bindings.willAttach(willAttach);
  if ($$props.hasCustomTextManager === void 0 && $$bindings.hasCustomTextManager && hasCustomTextManager !== void 0)
    $$bindings.hasCustomTextManager(hasCustomTextManager);
  ...
  if ($$props.getWebComponent === void 0 && $$bindings.getWebComponent && getWebComponent !== void 0)
    $$bindings.getWebComponent(getWebComponent);
  return `<vm-video${add_attribute("class", className, 0)}${add_attribute("style", style, 0)}${add_attribute("will-attach", willAttach, 0)}${add_attribute("has-custom-text-manager", hasCustomTextManager, 0)}${add_attribute("cross-origin", crossOrigin, 0)}${add_attribute("preload", preload, 0)}${add_attribute("poster", poster, 0)}${add_attribute("controls-list", controlsList, 0)}${add_attribute("auto-pip", autoPiP, 0)}${add_attribute("disable-pip", disablePiP, 0)}${add_attribute("disable-remote-playback", disableRemotePlayback, 0)}${add_attribute("media-title", mediaTitle, 0)}${add_attribute("this", __ref, 0)}>${slots.default ? slots.default({}) : ``}</vm-video>`;
});
...
const usePlayerStore = (ref) => {
  const internalStoreRef = new Map();
  const vimeable = (prop, initialValue) => {
    const store2 = writable(initialValue);
    const canWrite = isWritableProp(prop);
    const set = (value) => {
      if (!get_store_value(internalStoreRef.get("ready")))
        ;
    };
    const update = (updater) => {
      set(updater(get_store_value(store2)));
    };
    internalStoreRef.set(prop, store2);
    return {
      subscribe: store2.subscribe,
      update: canWrite ? update : void 0,
      set: canWrite ? set : void 0
    };
  };
  const store = Object.keys(initialState).reduce((prev, prop) => Object.assign(Object.assign({}, prev), { [prop]: vimeable(prop, initialState[prop]) }), {});
  return store;
};
...
var Column$1 = Column;
var featured_media_list_svelte_svelte_type_style_lang = "";
var media_player_component_svelte_svelte_type_style_lang = "";
export { ClickableTile$1 as C, DefaultUi as D, Grid$1 as G, Player as P, Row$1 as R, Scrim as S, Video as V, selected_media as a, selected_poster as b, Column$1 as c, selected_cookie as s, usePlayerStore as u };
